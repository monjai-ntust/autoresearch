...
class PhaseBScriptContractTests(unittest.TestCase):
    def test_full_run_derives_machine_specific_values(self):
        ...
        self.assertIn('export UV_CACHE_DIR="$SOURCE_ROOT/output/.uv-cache"', SCRIPT)
        self.assertIn('export UV_PROJECT_ENVIRONMENT="$SOURCE_ROOT/output/.uv-venv"', SCRIPT)
        self.assertIn('export UV_PYTHON_INSTALL_DIR="$SOURCE_ROOT/output/.uv-python"', SCRIPT)
        self.assertIn('export HF_HOME="$SOURCE_ROOT/$RUN_ROOT/inputs/huggingface"', SCRIPT)
        self.assertIn('export HF_XET_CACHE="$HF_HOME/xet"', SCRIPT)
        self.assertLess(SCRIPT.index("export UV_CACHE_DIR="), SCRIPT.rindex("\nuv sync --frozen"))
