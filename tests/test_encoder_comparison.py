...
from utils.encoder_comparison.trainer import historical_comparison_boost
...
class EncoderComparisonTests(unittest.TestCase):
    ...
    def test_unknown_profile_and_mutable_revision_fail_closed(self):
        with self.assertRaisesRegex(DataContractError, "unknown encoder"):
            self.config.select("not-reviewed", "historical-a20-a21-a12")
    ...
    def test_stage_reuses_current_preparation_and_data_cache_contracts(self):
        ...
        item = canonical_data.CodeAccordDataset(examples, _Tokenizer(), 16)[0]
        self.assertEqual(item["ner_labels"][0], -100)
        self.assertEqual(item["ner_labels"][1], canonical_data.BIO_TAG2ID["B-Object"])
        self.assertEqual(item["ner_labels"][2], canonical_data.BIO_TAG2ID["I-Object"])
        self.assertEqual(item["ner_labels"][3], -100)
        batch = canonical_data.collate_fn([item])
    ...
    def test_a20_middle_value_lasts_exactly_one_batch(self):
        at_gate = historical_comparison_boost(
            step=1000,
            max_steps=3500,
            initial=5.0,
            end=2.0,
        ...
        after_gate = historical_comparison_boost(
            step=1001,
            max_steps=3500,
            initial=5.0,
            end=2.0,
    ...
    def test_run_binding_rejects_other_profile_and_cross_run_manifest(self):
        with _temporary_output_directory() as temporary:
            ...
            first.create()
            second.create()
            large = self.config.select("deberta-large-v1", "historical-a20-a21-a12")
            base = self.config.select("deberta-base-v1", "historical-a20-a21-a12")
    ...
    def test_run_local_cache_uses_current_manifest_and_rejects_digest_mismatch(self):
        with _temporary_output_directory() as temporary:
            ...
            layout.create()
            selected = self.config.select("bert-base-uncased", "historical-a20-a21-a12")
    ...
    def test_checkpoint_summary_substitution_fails_before_manifest_promotion(self):
        with _temporary_output_directory() as temporary:
            ...
            selected = self.config.select(
                "deberta-large-v1", "historical-a20-a21-a12"
            )
