...
class OfflineScoringTests(unittest.TestCase):
    def test_four_conditions_match_manual_counts_and_collapse_correction_duplicate(self):
        ...
        with _temporary_output_directory() as temporary:
            ...
            publication_table = layout.resolve("metrics/publication-table.tsv")
            publication_summary = layout.resolve("metrics/publication-summary.md")
            self.assertTrue(publication_table.is_file())
            self.assertTrue(publication_summary.is_file())
            table_text = publication_table.read_text(encoding="utf-8")
            self.assertIn("condition\tcandidate_accuracy\ttriple_tp", table_text)
            self.assertIn("VER-RAW\t0.666667\t2\t1\t0", table_text)
            self.assertIn("NA", table_text)
            summary_text = publication_summary.read_text(encoding="utf-8")
            self.assertIn("Publication data coverage complete: `false`", summary_text)
            self.assertIn("CODE-STRICT-1 triple metrics", summary_text)
