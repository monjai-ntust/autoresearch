...
| --- | --- | --- | --- | --- |
| `train_span.py`, `models/bert_kg_encoder.py` — CODE-ACCORD encoder capacity/full-recipe and zh-Hant span results | Dataset-selected training writes checkpoints outside `output`; used by augmentation, GAN/Gumbel, and inference code; historical split and type-agnostic scorer are incompatible with `CODE-STRICT-1`. | `phase_b.py model train` plus a typed-record adapter. | Eligible-fixture loss/output parity; frozen `CODE-SPLIT-1`; output/checkpoint identity; no final-test selection; clean-clone dry run. | Retain as model implementation, not an executable paper command. |
| `inference_kg.py` — encoder candidate/KG inputs and confidence ablations | Reads historical checkpoints; writes arbitrary JSONL; feeds the free-form verifier and graph builder. | `phase_b.py model generate-candidates` (`model.py`). | Live encoder inference under the pinned accelerator profile plus fixture parity for eligible typed spans and confidence ordering; candidate IDs already tied to strict data/split/model hashes. | Retain pending live/parity gate. `dry-run` and `replay` executions now exist and reproduce the greedy non-overlap selection and softmax-product confidence; live inference is not yet implemented, so this script is not yet removable. |
