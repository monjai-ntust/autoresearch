...
| `inference_kg.py` — encoder candidate/KG inputs and confidence ablations | Reads historical checkpoints; writes arbitrary JSONL; feeds the free-form verifier and graph builder. | `phase_b.py model generate-candidates`. | Fixture parity for eligible typed spans and confidence ordering; candidate IDs tied to strict data/split/model hashes. | Retain pending adapter. |
