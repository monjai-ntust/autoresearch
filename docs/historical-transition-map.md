...
| `provenance/inference_kg.py` — encoder candidate/KG inputs and confidence ablations | Reads historical checkpoints; writes arbitrary JSONL; feeds the free-form verifier and graph builder. | `phase_b.py model generate-candidates` (`model.py`). | Output parity between the `live` adapter and the historical script on a real checkpoint under the accelerator (verifiable only externally); candidate IDs already tied to strict data/split/model hashes. | Retain pending external parity validation. `dry-run`, `replay`, and `live` executions now exist; `live` runs the retained encoder and its imports/signatures/label space are statically verified, but its outputs cannot be validated offline, so this script is retained as provenance until external parity is confirmed. |
| `provenance/verify_triples_llm.py` — SciERC verifier row and CODE audit lineage | Free-form SciERC ontology/parser; arbitrary JSONL I/O; imported parser helpers are regression-tested. | `phase_b.py verifier dry-run|live|replay`. | Transport-only parity if reused; frozen prompt/schema/model digest/cache/telemetry tests already cover canonical semantics. | Demote to immutable provenance; remove only after parser fixtures move to canonical/legacy fixtures. |
| `provenance/build_kg.py`, `provenance/eval_graph_rag.py`, `provenance/diagnose_evidence_paths.py` — diagnostic Graph RAG and evidence-coverage claims | Chain consumes historical inference/verdict files; its answer-bearing gold query and unique-token overlap make it diagnostic provenance rather than valid accuracy. | C-04 `phase_b.sh --stage graph-rag` after canonical score. | Implemented: hash-bound Phase B condition adapter, head-only query, private target/support scoring, genuine BM25/graph/hybrid controls, matched budgets, diagnostic-only label, and focused leakage/recompute tests. External execution is still required for the retained full run. | Retain the old chain only as immutable methodology provenance; it is unreachable from the canonical replacement and cannot validate historical hit rates. |
| `provenance/train_multi.py`; `data/download_scierc.py`, `data/download_conll04.py`, `data/download_ade.py` — cross-dataset comparator | Token-BIO runner/downloaders have separate schemas and mutable acquisition. | `phase_b.py secondary benchmark`. | Per-dataset immutable input manifest, type/matcher declaration, matrix row, and fixture adapter. | Retain pending benchmark adapters. |
| `provenance/generate_accord_llm_aug.py`, `provenance/generate_entigraph.py`, `provenance/generate_cycle_data.py`, `provenance/generate_paraphrase_dataset.py`, `provenance/generate_synth_dataset.py` — augmentation/coverage-gap variants | Local models or incomplete inputs; generated JSONL normally writes under `results/`; `provenance/generate_accord_llm_aug.py` imports `train_span`. | `phase_b.py secondary generate-data`. | Input/prompt/model hashes, output containment, matched gold-only control, and claim-family reconciliation. | Retain pending one matrix-bound generator contract per supported family. |
| `provenance/generate_entity_masks.py`, `provenance/train_pretrain_cooperative.py`, `data/download_arxiv_real.py` — cooperative masking/pretraining negative result | Requires mutable arXiv input and historical checkpoint; writes arbitrary checkpoints/results. | `phase_b.py secondary cooperative-pretrain` or unsupported claim. | Reacquirable licensed input manifest, adapter parity, or discrepancy entry removing/qualifying the claim. | Retain as secondary provenance. |
| `provenance/train_stage2b.py`, `provenance/train_stage2c.py`, `provenance/train_stage2d.py`, `provenance/train_stage2e.py`, `provenance/train_gan.py`, `provenance/train_gumbel.py` and transitive decoder/critic modules — closed-loop negative-result families | Multiple unavailable checkpoints/models and direct writes; GAN/Gumbel import `train_span`; ledgers do not provide a complete replayable matrix. | `phase_b.py secondary closed-loop` with blocked/unsupported matrix rows. | Each claimed intervention mapped to an evidence bundle; valid fixture parity or explicit noncomparability; no orphan README command. | Retain pending claim-by-claim disposition. |
...
Accordingly, the Graph RAG (`provenance/build_kg.py`, `provenance/eval_graph_rag.py`,
`provenance/diagnose_evidence_paths.py`), cross-dataset (`provenance/train_multi.py`, downloaders),
augmentation (`generate_*`), cooperative-pretraining, closed-loop
(`provenance/train_stage2*`, `provenance/train_gan.py`, `provenance/train_gumbel.py`), and zh-Hant
(`provenance/zh_translate_project.py`, `provenance/dapt_zh.py`) rows are **retained as provenance
...
regenerated or deleted this cycle. `train_span.py`/`provenance/inference_kg.py` likewise stay
