...
| A-CLAIM-010 | Graph RAG diagnostic harness (cloze questions, overlap scoring) | **Retained (provenance)** | C-04 `graph-rag` | `provenance/eval_graph_rag.py` remains methodology provenance and is never invoked by C-04. The canonical replacement is a separately identified, leakage-safe Phase-B-output Regime-D diagnostic; it does not reproduce or validate the historical hit rates. |
