...
| Graph RAG diagnostic (`S5-2-GRAPH-RAG`; A-CLAIM-022; DISC-007/008) | n=10 QA hit rates, 50–60% gold ceiling, "BM25" text baseline | (c) historical values remain unsupported leakage-prone/not-BM25 provenance; C-04 is a new non-comparable diagnostic | post-score `phase_b.sh --stage graph-rag` | Source implementation and read-only historical readiness pass; an external machine must run the child over the retained full-run parent before any C-04 result exists. Regime Q remains blocked by the no-domain-expert gate. | None new — C-04 was explicitly authorized with head-only private-scored Regime D, 585 eligible probes, genuine matched BM25/graph/hybrid controls, and clustered uncertainty. Do not reinterpret it as QA or as validation of the old values. |
