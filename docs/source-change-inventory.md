...
| `docs/phase_b_workflow.md` | Gives standalone commands, one copy/paste fresh-clone/no-checkpoint bootstrap block with an explicit post-`prepare` stop boundary, later-stage verifier prerequisites, stage gates, output layout, non-enforcing environment-version recording, and reproducibility limitations for the canonical runner. |
