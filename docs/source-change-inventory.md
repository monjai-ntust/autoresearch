...
| `scripts/phase_b_debug.sh` | Bash recovery runner for the standalone external checkout. Its default sweep parser-checks every command and reports expensive external training as blocked; `train-live` runs/resumes the selected canonical seed, while `publishable` advances that seed through development candidates and then stops at B-07. Its acknowledged `smoke` stage reuses completed seed-42 artifacts, runs one real test inference plus both live Ollama verifier modes, and namespaces pseudo-seed downstream checks as permanently nonpublication. For a new run ID it hash-verifies and hard-links (or copies as a fallback) a prior run's immutable CODE-ACCORD archive before re-running fetch's per-run manifest verification, avoiding unnecessary re-downloads without sharing derived artifacts. The acknowledged legacy CSV diagnostic remains noncanonical. Completion checks are seed-specific. |
