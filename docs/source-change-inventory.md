...
| `scripts/phase_b_debug.sh` | Bash recovery runner for the standalone external checkout. It pulls `refactor`, synchronizes the lockfile, detects completed run-local output contracts, resumes only at the first incomplete selected command, and stops at the first new error. It exposes every implemented canonical command family plus an explicitly acknowledged, manifest-marked noncanonical `train_span.py` diagnostic; the publishable chain still blocks at the missing CODE-SPLIT-1 live-training adapter. |
