...
| `docs/phase_b_graph_rag_diagnostic.md` | Freezes C-04's authority/claims boundary, exact Phase B parent artifact set, condition derivation, head-only private-scored Regime-D protocol, matched controls, uncertainty, child identity/resume rules, historical readiness facts, no-network route, external recovery command, and evidence-based retention audit. |
...
| `phase_b.sh` (moved from `scripts/phase_b_debug.sh`) | Primary publication and recovery launcher for the standalone external checkout. It derives a run ID, upstream, seeds, Python environment, Ollama model, and blob identity instead of pinning machine-local values. Its `full` stage runs the unchanged independent eight-seed development/pilot/test/verifier/score chain with implicit invocation authorization but a mandatory passing B-07 pilot audit, then runs C-04 only after score completion. The recovery-only `graph-rag` stage requires that score and invokes no Phase B producer. The launcher stops at the first new error, resumes training or partial final-verifier responses, and otherwise performs validated stage-scoped cleanup recorded in a recovery manifest. Availability, smoke, replay, verified archive reuse, seed-specific completion checks, and acknowledged legacy CSV modes remain additional diagnostic/recovery features. |
...
| `docs/migration.md` | Records the Phase B design-only raw/typed migration boundary and the evidence required before model adapters can become canonical. Its legacy `C-MIGRATION-*` revision keys predate the current Phase C task numbering and remain immutable historical identifiers. |
...
| Path | Current role and reason |
| --- | --- |
| `graph_rag.py` | C-04 canonical diagnostic and noncanonical C-01I contract/readiness CLI. `phase-b-diagnostic` is the sole result-producing route and consumes one scored Phase B run; `doctor`, `prepare`, and `evaluate` retain synthetic/no-expert/readiness validation without importing the historical benchmark or a parent-repository resource. |
| `graph_rag_eval/phase_b_artifacts.py`<br>`graph_rag_eval/phase_b_diagnostic.py` | C-04 adapter and runner. They hash-verify the complete scored parent family read-only, independently reconstruct all three Phase B graph conditions, create leakage-safe head-only Regime-D probes with private targets, build deterministic per-seed graphs, execute matched-budget BM25/graph/hybrid plus a private oracle, recompute intrinsic/retrieval metrics and clustered intervals, and use an immutable create-only `graph-rag/` child identity. |
| `graph_rag_eval/__init__.py`<br>`graph_rag_eval/capabilities.py`<br>`graph_rag_eval/contracts.py`<br>`graph_rag_eval/identity.py`<br>`graph_rag_eval/registry.py`<br>`graph_rag_eval/trace.py`<br>`graph_rag_eval/budget.py`<br>`graph_rag_eval/runner.py` | Dataset-neutral contract/readiness core reused by C-04 for canonical records, graph snapshots, retrieval, budgets, statistics, schema/output containment, and source identities. The generic runner retains typed capability failures, configuration-selected adapters/generators, preparation, layered offline execution, no-domain-expert hard stops, and synthetic validation; it is not a canonical result-producing route. C-04 corrects its namespace binding so a verified child can coexist with an existing Phase B parent while an occupied child still requires exact identity. |
| `graph_rag_eval/adapters/__init__.py`<br>`graph_rag_eval/adapters/base.py`<br>`graph_rag_eval/adapters/synthetic.py`<br>`graph_rag_eval/adapters/code_accord.py` | Adapter boundary. `base.GeneratedGraph` lets an adapter declare a predicted graph independent of gold so intrinsic precision is measurable; `base.gold_subset_graph` remains for recall-only adapters and fixes precision at 1.0 by construction. The synthetic adapter supplies deterministic answerable/unanswerable and Regime D/Q fixtures plus a predicted graph with both a missed entity/triple and a hallucinated entity, relation, and triple. The CODE-ACCORD adapter owns its raw paths/BIO interpretation, declares an empty predicted graph because no extraction output exists, and reports the incomplete tracked corpus plus unavailable independent Regime Q author/reviewer as blockers instead of manufacturing questions or zeros. |
| `graph_rag_eval/graphs/__init__.py`<br>`graph_rag_eval/graphs/snapshots.py`<br>`graph_rag_eval/graphs/corruptions.py`<br>`graph_rag_eval/graphs/matching.py` | Content-addressed graph snapshots, reference-free structural summaries (degree, components, isolated nodes, clustering, provenance coverage), exact/relaxed one-to-one matching, and deterministic edge add/drop, endpoint rewire, relation relabel, provenance removal, entity merge, and entity split controls with parent/recipe identity. |
| `graph_rag_eval/retrieval/__init__.py`<br>`graph_rag_eval/retrieval/base.py`<br>`graph_rag_eval/retrieval/bm25.py`<br>`graph_rag_eval/retrieval/dense.py`<br>`graph_rag_eval/retrieval/graph.py`<br>`graph_rag_eval/retrieval/hybrid.py` | One `QueryView` retrieval boundary; genuine frozen-parameter `bm25s` ranking; deterministic offline dense-shaped fixture plus pinned local-cache Transformer dense execution; question-only graph seed/expansion with provenance and question-overlap tie-breaking instead of triple-ID order; and deterministic reciprocal-rank fusion that reports every child failure and child seed/expansion trace. |
| `graph_rag_eval/evaluation/__init__.py`<br>`graph_rag_eval/evaluation/intrinsic.py`<br>`graph_rag_eval/evaluation/retrieval.py`<br>`graph_rag_eval/evaluation/generation.py`<br>`graph_rag_eval/evaluation/coupled.py`<br>`graph_rag_eval/evaluation/statistics.py` | Separates intrinsic graph, evidence retrieval, answer/support/abstention, coupled, and uncertainty layers. `success_at_k` keeps the KILT "at least one relevant item" definition and whole-set containment is reported separately as `sufficient_evidence_at_k`; correct abstention is scored apart from answer accuracy; unmet prerequisites emit typed records with no numeric field. The generation module hosts the configuration-driven offline rule generator and the pinned local-cache Transformer interface behind one `build_prompt`/`generate` contract; judge metrics remain gated on unavailable human calibration. |
| `configs/phase_b_graph_rag_synthetic.json` | Freezes the offline contract fixture, public/private projection, conditions, BM25/graph parameters, matched budget, corruptions, generator import path/identity and its fixture answer rules, prompt template, seed, and 10,000-resample clustered bootstrap. |
| `configs/phase_b_graph_rag_code_accord.json` | Freezes CODE-ACCORD readiness inputs, names the pinned Transformer generator host, and explicitly records that complete gold inputs, independent Regime Q author/reviewer, model/prompt selection, calibration, minimum effect, and comparison family are unresolved; the runner consequently fails closed before a live call. |
| `configs/phase_b_graph_rag_diagnostic.json` | Freezes the C-04 parent identities and artifact paths, three Phase B condition mappings, head-only leakage-safe probe, matched BM25/graph/hybrid budget, private oracle ceiling, denominators/failures, confidence-referenced comparisons, and source-document-clustered bootstrap. |
...
| `schemas/phase_b/rag-phase-b-diagnostic-config.schema.json` | Validates the closed C-04 parent/condition/probe/retrieval/statistics contract and rejects unknown fields. |
| `schemas/phase_b/rag-phase-b-child-identity.schema.json` | Validates the immutable Graph RAG child binding to one canonical Phase B parent, config, source surface, and schema set. |
| `schemas/phase_b/rag-phase-b-diagnostic-output.schema.json` | Validates C-04 run, artifact-ledger, and terminal completion manifests. |
...
| `tests/test_graph_rag_contracts.py`<br>`tests/test_graph_rag_adapters.py`<br>`tests/test_graph_rag_identity.py`<br>`tests/test_graph_rag_registry_isolation.py`<br>`tests/test_graph_rag_bm25.py`<br>`tests/test_graph_rag_graph_retrieval.py`<br>`tests/test_graph_rag_budget.py`<br>`tests/test_graph_rag_leakage.py`<br>`tests/test_graph_rag_metrics.py`<br>`tests/test_graph_rag_artifacts.py`<br>`tests/test_graph_rag_runner.py`<br>`tests/test_graph_rag_standalone.py` | C-01I focused suite: canonical/schema rejection, adapter capabilities, cache/dataset isolation, temporary third-adapter zero-core-change proof, hand-worked BM25 behavior, graph expansion/provenance/corruptions/matching, matched budgets, leakage sentinels, layered metrics/bootstrap, deterministic artifact replay, no-domain-expert hard stop, and execution from a copied standalone checkout with no parent/paper/research submodule. |
| `tests/phase_b_diagnostic_fixture.py`<br>`tests/test_graph_rag_phase_b_diagnostic.py` | Creates a miniature hash-bound scored Phase B parent and tests C-04 ingestion/condition reconstruction, parent hash refusal, child containment, partial/completed recovery, leakage sentinel, deterministic graph derivation, gold-oracle separation, no-overwrite behavior, and independent metric/denominator recomputation. |
