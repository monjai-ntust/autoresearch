...
| `scripts/phase_b_debug.sh` | Bash recovery runner for the standalone external checkout. Its default sweep parser-checks every documented command, runs every stage whose run-local inputs exist, records output-backed completion, reports gated/missing prerequisites as `BLOCKED`, and stops on any attempted-command error. It also exposes an explicitly acknowledged, manifest-marked noncanonical `train_span.py` diagnostic; the strict publishable chain still blocks at the missing CODE-SPLIT-1 live-training adapter. |
