...
| `scripts/phase_b_debug.sh` | Bash recovery runner for the standalone external checkout. Its default sweep parser-checks every documented command, runs every stage whose run-local inputs exist, records output-backed completion, reports gated/missing prerequisites as `BLOCKED`, and returns nonzero if any blocks remain or an attempted command errors. Its acknowledged noncanonical `train_span.py` diagnostic locates the extracted CSV root dynamically and preflights the GB10/CUDA runtime before loading the model; the strict publishable chain still blocks at the missing CODE-SPLIT-1 live-training adapter. |
