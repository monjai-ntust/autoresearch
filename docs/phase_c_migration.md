...
| `phase_b_pipeline/verifier.py`, `prompts/phase_b/`, and verifier schemas | Reuse | Preserve the frozen prompt-bundle/model/decoding identities, exact-token correction mapping, warm-up exclusion, three-attempt policy, cache semantics, and separation of dry-run/live/replay. A protocol revision requires new prompt/schema hashes rather than in-place drift. |
